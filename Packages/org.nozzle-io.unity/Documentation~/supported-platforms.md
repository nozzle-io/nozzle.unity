...
The package currently contains C# bindings plus a native bridge ABI. The Git package does not commit compiled native plugins; CI-staged stub/native ABI artifacts may contain a compiled `nozzle_unity` bridge binary under `Runtime/Plugins/<platform>/`. The default bridge build reports `runtime_supported = 0`. Support claims require a compiled native Unity bridge, nozzle core integration, and Editor/Player smoke evidence.
